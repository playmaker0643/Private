      ...
      <header className="relative pt-12 pb-8 text-center">
        <div className={`transform transition-all duration-1000 ${animateElements ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
          ...
          <div className="mt-6 flex justify-center gap-2">
            {[...Array(5)].map((_, i) => (
              <div key={i} className={`w-2 h-2 rounded-full bg-emerald-500 transform transition-all duration-700`} 
                style={{ animationDelay: `${i * 100}ms`, animation: 'pulse 2s ease-in-out infinite' }}></div>
            ))}
          </div>
        </div>
      </header>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {greetings.map((greeting, idx) => (
                <div
                  ...
                  style={{
                    animation: animateElements ? `slideUp 0.6s ease-out ${idx * 100}ms forwards` : 'none',
                    opacity: 0,
                  }}
                >
                  <div className="bg-white rounded-2xl p-8 shadow-md hover:shadow-xl transition-all duration-300 border border-emerald-100 hover:border-emerald-300">
                    <div className="text-5xl md:text-6xl font-bold text-emerald-600 mb-4 font-arabic">
                      {greeting.arabic}
                    </div>
            ...
            <div className="space-y-6">
              {messages.map((message, idx) => (
                <div
                  ...
                  className="group"
                  style={{
                    animation: animateElements ? `slideIn 0.6s ease-out ${idx * 150}ms forwards` : 'none',
                    opacity: 0,
                  }}
                >
